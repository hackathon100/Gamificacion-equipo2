...
const Step = ({ title, text, dialog, choices, image, name, index, setCurrentStep, onSendInput }) => {
        ...
        {choices ? (
          <div className="mt-3 step-choices d-flex flex-column">
            {_.map(choices, (choice) => {
              ...
              return <Button className="mt-2" onClick={() => setCurrentStep(choice.next ? choice.next : index + 1)} variant="contained" color="primary">{choice.text}</Button>
            })}
          </div>
        ) : null}
      </div>
